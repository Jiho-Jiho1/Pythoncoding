...
# --------------------------------------------------------------------
    ...
    def set_pos(self, x, y):
        """Sets new spot of robot's position."""
        self._x=x
        self._y=y
        self._update_pos()
        self._refresh()
    ...
    def get_dir(self):
        """Tells you the current direction of the robot."""
        dirs = 'NWSE'
        return dirs[self._dir]
    
    def set_dir(self, dir):
        """Changes the current direction of the robot."""
        if type(dir) is str:
            if dir in 'NWSE':
                self.image[self._dir].moveTo(-100, -100)
                self._dir = _orient_dict[dir]
                self._update_pos()
                self._update_trace()
                self._refresh()
...
if __name__ == "__main__":
    if len(_sys.argv) > 1:
        ...
        create_world()
    edit_world()
    r = Robot(color="yellow", beepers=100)
    r.set_trace("blue")
    while True:
        try:
            e = _scene.wait()
